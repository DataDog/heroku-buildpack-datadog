...
### Order of buildpacks
As explained in the [Heroku][4] documentation, the last buildpack in the list is used to determine the process type for the application.
...
For additional documentation, see the [Datadog Agent][12] documentation.
...
To run any of the information/debugging commands listed in the [Agent][22] documentation use the `agent-wrapper` command.
...
Prior to version `6.14` the Datadog v6 Agent shipped with Python version `2` embedded. Starting with `6.14`, and in preparation for Python version `2` End Of Life, announced for January 2020, the Datadog v6 Agent ships with both Python versions `2` and `3`, to give customers enough time to migrate their custom checks to Python version `3`. The Heroku buildpack only keeps one of the versions. Set `DD_PYTHON_VERSION` to `2` or `3` to select the Python version you want the Agent to keep. If not set, the buildpack keeps Python version `2`. If you are using custom checks that only work with Python version `2`, Datadog recommends migrating them to version `3` before its EOL.
...
See the [contributing][25] documentation to learn how to open an issue or PR to the [Heroku-buildpack-datadog repository][26].
